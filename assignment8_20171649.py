...
class Calculator(QWidget):
    ...
    def buttonClicked(self):
        ...
        funcList = {'factorial (!)' : calcFunctions.factorial(self.display.text()),'-> binary' : calcFunctions.decToBin(self.display.text()),
                       'binary -> dec': calcFunctions.binToDec(self.display.text()),'-> roman' : calcFunctions.decToRoman(self.display.text())}
        ...
        if key == '=':
            ...
            for conskey in consList.keys():
                if key == conskey:
                    self.display.setText(self.display.text() + str(consList[key]))
